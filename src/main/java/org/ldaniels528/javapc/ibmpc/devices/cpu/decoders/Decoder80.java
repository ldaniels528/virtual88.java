...
import static org.ldaniels528.javapc.ibmpc.devices.cpu.decoders.DecoderUtil.lookupOperands;
import static org.ldaniels528.javapc.ibmpc.devices.cpu.decoders.DecoderUtil.lookupRegister;
...
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.addressing.LEA;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.bitwise.AND;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.bitwise.OR;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.bitwise.XOR;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.data.MOV;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.data.XCHG;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.ADC;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.ADD;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.CMP;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.SBB;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.SUB;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.TEST;
...
public class Decoder80 implements Decoder {
	 ...
	 * @return an {@link OpCode 8086 opCode}
	 */
	private OpCode decodeTypeA( final I8086 cpu, final X86MemoryProxy proxy, final int code16 ) {
		...
		final Operand target = DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16);
		final Operand source = ( groupCode == 1 ) ? DecoderUtil.nextSignedValue8(proxy) : DecoderUtil.nextValue(proxy, memClass);
	...
	private OpCode decodeTypeC( final I8086 cpu, final X86MemoryProxy proxy, final int code16 ) {
		...
		final Operand[] operands = DecoderUtil.lookupOperands(cpu, proxy, code16);
	...
	private LEA decodeLEA( final I8086 cpu, final X86MemoryProxy proxy, final int code16 ) {
		...
		final X86Register dst = DecoderUtil.lookupRegister(cpu, proxy, code16);
			
		// lookup the secondary operand (memory reference)	
		final MemoryReference src = DecoderUtil.lookupReferencedAddress(cpu, proxy, compCode);
	...
	private MOV decodeMOV( final I8086 cpu, final X86MemoryProxy proxy, final int code16 ) {
		...
		final Operand[] operands = DecoderUtil.lookupOperands(cpu, proxy, code16, true, true);
