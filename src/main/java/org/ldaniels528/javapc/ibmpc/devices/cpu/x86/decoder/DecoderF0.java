...
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.bitwise.NOT;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.data.DB;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.data.LOCK;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.flags.*;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.flow.callret.CALL;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.flow.jump.JMP;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.math.*;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.stack.PUSH;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.string.REPNZ;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.string.REPZ;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.system.HLT;
    ...
    private OpCode decodeTypeA(final I8086 cpu, final X86MemoryProxy proxy, final int code16, final int subCode) {
        ...
        switch (subCode) {
            case INS_TYPE_A_TEST:
                return new TEST(operand, nextValue(proxy, memClass));
            case INS_TYPE_A_NOT:
    ...
    private OpCode decodeTypeB(final I8086 cpu, final X86MemoryProxy proxy, int code16, final int subCode) {
        ...
        switch (subCode) {
            case INS_TYPE_B_CALL:
                return new CALL(lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_CALL_FAR:
                return new CALL(lookupSecondaryOperand(cpu, proxy, code16, SIZE_32BIT));
            case INS_TYPE_B_JMP:
                return new JMP(lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_JMP_FAR:
                return new JMP(lookupSecondaryOperand(cpu, proxy, code16, SIZE_32BIT));
            case INS_TYPE_B_PUSH:
                return new PUSH(lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_INC:
                return new INC(lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_DEC:
                return new DEC(lookupSecondaryOperand(cpu, proxy, code16));
            default:
                throw new UnhandledByteCodeException(code16);
        }
    }
