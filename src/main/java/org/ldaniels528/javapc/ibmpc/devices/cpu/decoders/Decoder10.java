...
public class Decoder10 implements Decoder {
	...
	public OpCode decode(final I8086 cpu, final X86MemoryProxy proxy, DecodeProcessor processor) {
		...
		switch( code8 ) {
			// ADC AL,nn
			case 0x14:	return new ADC( cpu.AL, DecoderUtil.nextValue8(proxy) );
			// ADC AX,nnnn
			case 0x15:	return new ADC( cpu.AX, DecoderUtil.nextValue16(proxy) );
			// PUSH SS
			case 0x16:	return new PUSH( cpu.SS );
			// POP SS
			case 0x17:	return new POP( cpu.SS );
			// SBB AL,nn
			case 0x1C:	return new SBB( cpu.AL, DecoderUtil.nextValue8(proxy) );
			// SBB AX,nnnn
			case 0x1D:	return new SBB( cpu.AX, DecoderUtil.nextValue16(proxy) );
			// PUSH DS
