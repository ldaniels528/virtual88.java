...
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.data.MOVSX;
...
public class Decoder00 implements Decoder {
    ...
    public OpCode decode(final I8086 cpu, final X86MemoryProxy proxy, DecodeProcessor processor) {
        ...
        switch (code8) {
            // ADD AL,nn
            case 0x04:
                return new ADD(cpu.AL, DecoderUtil.nextValue8(proxy));
            // ADD AX,nnnn
            case 0x05:
                return new ADD(cpu.AX, DecoderUtil.nextValue16(proxy));
            // PUSH ES
            case 0x06:
            ...
            // OR AL,nn
            case 0x0C:
                return new OR(cpu.AL, DecoderUtil.nextValue8(proxy));
            // OR AX,nnnn
            case 0x0D:
                return new OR(cpu.AX, DecoderUtil.nextValue16(proxy));
            // PUSH CS
            case 0x0E:
    ...
    private OpCode decode0F(final I8086 cpu, final X86MemoryProxy proxy) {
        ...
        switch (subCode) {
            // MOVSX r16/32,r/m8
            case 0xBE:
                return decodeMOVSX(cpu, proxy, DecoderUtil.MEM_CLASS_8BIT);
            // MOVSX r32,r/m16
            case 0xBF:
                return decodeMOVSX(cpu, proxy, DecoderUtil.MEM_CLASS_16BIT);
            // POP CS
            default:
                proxy.moveOffset(-1);
    ...
    private X86Register decodeRegister16(final I8086 cpu, final X86MemoryProxy proxy) {
        ...
        return DecoderUtil.lookupRegister(cpu, proxy, regID);
    }
    ...
    private Operand[] decodeOperands(final I8086 cpu,
        ...
        final X86Register dest = DecoderUtil.getRegister(cpu, regCode, SIZE_16BIT);

        // get the source operand
        final Operand src = DecoderUtil.lookupOperand(cpu, proxy, elemCode, refCode, srcMemClass);
