...
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.flags.*;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.*;
import org.ldaniels528.javapc.ibmpc.devices.cpu.operands.Operand;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.bitwise.NEG;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.bitwise.NOT;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.data.DB;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.data.LOCK;
...
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.string.REPNZ;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.string.REPZ;
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.system.HLT;
...
import static org.ldaniels528.javapc.ibmpc.devices.cpu.decoders.DecoderUtil.lookupSecondaryOperand;
...
public class DecoderF0 implements Decoder {
    ...
    public OpCode decode(final I8086 cpu, final X86MemoryProxy proxy, DecodeProcessor processor) {
        ...
        switch (code8) {
            ...
            // REPNZ
            case 0xF2:
                return new REPNZ(this.processor.decodeNext());
            // REPZ
            case 0xF3:
                return new REPZ(this.processor.decodeNext());
            // HLT
            case 0xF4:
                return HLT.getInstance();
            // CMC
            case 0xF5:
                return CMC.getInstance();
            // CLC
            case 0xF8:
                return CLC.getInstance();
            // STC
            case 0xF9:
                return STC.getInstance();
            // CLI
            case 0xFA:
                return CLI.getInstance();
            // STI
            case 0xFB:
                return STI.getInstance();
            // CLD
            case 0xFC:
                return CLD.getInstance();
            // STD
            case 0xFD:
    ...
    private OpCode decodeComplexCode(final I8086 cpu, final X86MemoryProxy proxy, final int code8) {
        ...
        switch (insCode) {
            case INS_TYPE_A:
                return decodeTypeA(cpu, proxy, code16, subCode);
            case INS_TYPE_B:
                return decodeTypeB(cpu, proxy, code16, subCode);
            default:
     ...
     */
    private OpCode decodeTypeA(final I8086 cpu, final X86MemoryProxy proxy, final int code16, final int subCode) {
        // extract the memory class
        // code: .... .... .... ...c (mask = 0000 0000 0000 0001)
        final int memClass = (code16 & 0x0001);

        // lookup the secondary operand
        final Operand operand = DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16);

        // decode the instruction
        switch (subCode) {
            case INS_TYPE_A_TEST:
                return new TEST(operand, DecoderUtil.nextValue(proxy, memClass));
            case INS_TYPE_A_NOT:
                return new NOT(operand);
            case INS_TYPE_A_NEG:
                return new NEG(operand);
            case INS_TYPE_A_MUL:
                return new MUL(operand);
            case INS_TYPE_A_IMUL:
                return new IMUL(operand);
            case INS_TYPE_A_DIV:
                return new DIV(operand);
            case INS_TYPE_A_IDIV:
     ...
     * @return the {@link OpCode decoded instruction}
     */
    private OpCode decodeTypeB(final I8086 cpu, final X86MemoryProxy proxy, int code16, final int subCode) {
        // decode the instruction
        switch (subCode) {
            case INS_TYPE_B_CALL:
                return new CALL(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_CALL_FAR:
                return new CALL(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16, SIZE_32BIT));
            case INS_TYPE_B_JMP:
                return new JMP(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_JMP_FAR:
                return new JMP(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16, SIZE_32BIT));
            case INS_TYPE_B_PUSH:
                return new PUSH(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_INC:
                return new INC(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16));
            case INS_TYPE_B_DEC:
                return new DEC(DecoderUtil.lookupSecondaryOperand(cpu, proxy, code16));
            default:
                throw new UnhandledByteCodeException(code16);
        }
    }
