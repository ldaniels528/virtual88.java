...
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.string.*;
...
public class DecoderA0 implements Decoder {
    ...
    public OpCode decode(final I8086 cpu, final X86MemoryProxy proxy, DecodeProcessor processor) {
        ...
        switch (code8) {
            // MOV AL,[nnnn]
            case 0xA0:
                return new MOV(cpu.AL, DecoderUtil.nextAddressByte(cpu, proxy));
            // MOV AX,[nnnn]
            case 0xA1:
                return new MOV(cpu.AX, DecoderUtil.nextAddressWord(cpu, proxy));
            // MOV [nnnn],AL
            case 0xA2:
                return new MOV(DecoderUtil.nextAddressByte(cpu, proxy), cpu.AL);
            // MOV [nnnn],AX
            case 0xA3:
                return new MOV(DecoderUtil.nextAddressWord(cpu, proxy), cpu.AX);
            // MOVSB
            case 0xA4:
            ...
            // TEST AL,nn
            case 0xA8:
                return new TEST(cpu.AL, DecoderUtil.nextAddressByte(cpu, proxy));
            // TEST AX,nnnn
            case 0xA9:
                return new TEST(cpu.AX, DecoderUtil.nextAddressWord(cpu, proxy));
            // STOSB
            case 0xAA:
