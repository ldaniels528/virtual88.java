...
import org.ldaniels528.javapc.ibmpc.devices.cpu.opcodes.math.AAA;
...
public class Decoder30 implements Decoder {
    ...
    public OpCode decode(final I8086 cpu, final X86MemoryProxy proxy, DecodeProcessor processor) {
        ...
        switch (code8) {
            // XOR AL,nn
            case 0x34:
                return new XOR(cpu.AL, DecoderUtil.nextValue8(proxy));
            // XOR AX,nnnn
            case 0x35:
                return new XOR(cpu.AX, DecoderUtil.nextValue16(proxy));
            // SS:
            case 0x36:
            ...
            // CMP AL,nn
            case 0x3C:
                return new CMP(cpu.AL, DecoderUtil.nextValue8(proxy));
            // CMP AX,nnnn
            case 0x3D:
                return new CMP(cpu.AX, DecoderUtil.nextValue16(proxy));
            // DS:
            case 0x3E:
    ...
    private OpCode decodeComplexCode(final I8086 cpu, final X86MemoryProxy proxy, final int code8) {
        ...
        // get data elements
        final Operand[] operands = DecoderUtil.lookupOperands(cpu, proxy, code16); //lookupOperands( cpu, proxy, code16, true, false );
