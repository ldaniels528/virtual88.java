...
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.flow.loop.LOOP;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.flow.loop.LOOPNZ;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.flow.loop.LOOPZ;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.io.IN;
import org.ldaniels528.javapc.ibmpc.devices.cpu.x86.opcodes.io.OUT;
...
public class DecoderE0 implements Decoder {
    ...
    public OpCode decode(final Intel8086 cpu, final X86MemoryProxy proxy) {
        ...
        switch (code8) {
            // LOOPNZ relOffset
            case 0xE0:
                return new LOOPNZ(nextAddressShort(proxy));
            // LOOPZ relOffset
            case 0xE1:
                return new LOOPZ(nextAddressShort(proxy));
            // LOOP relOffset
            case 0xE2:
                return new LOOP(nextAddressShort(proxy));
            // JCXZ relOffset
            case 0xE3:
                return new JCXZ(nextAddressShort(proxy));
            // IN AL,nn
            case 0xE4:
                return new IN(cpu.AL, nextValue8(proxy));
            // IN AX,nn
            case 0xE5:
                return new IN(cpu.AX, nextValue8(proxy));
            // OUT nn,AL
            case 0xE6:
                return new OUT(nextValue8(proxy), cpu.AL);
            // OUT nn,AX
            case 0xE7:
                return new OUT(nextValue8(proxy), cpu.AX);
            // CALL offset
            case 0xE8:
                return new CALL(nextAddressNear(proxy));
            // JMP offset
            case 0xE9:
                return new JMP(nextAddressNear(proxy));
            // JMP segment:offset
            case 0xEA:
                return new JMP(nextAddressFar(cpu, proxy));
            // JMP relOffset
            case 0xEB:
                return new JMP(nextAddressShort(proxy));
            // IN AL,DX
            case 0xEC:
