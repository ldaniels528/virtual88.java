...
import ibmpc.devices.cpu.x86.opcodes.addressing.DS;
import ibmpc.devices.cpu.x86.opcodes.addressing.SS;
import ibmpc.devices.cpu.x86.opcodes.bitwise.XOR;
import ibmpc.devices.cpu.x86.opcodes.math.AAA;
...
public class Decoder30 implements Decoder {
	...
	public OpCode decode( final Intel80x86 cpu, final X86MemoryProxy proxy ) {
		...
		switch( code8 ) {
			// XOR AL,nn
			case 0x34:	return new XOR( cpu.AL, nextValue8( proxy ) );
			// XOR AX,nnnn
			case 0x35:	return new XOR( cpu.AX, nextValue16( proxy ) );
			// SS:
			case 0x36:	return SS.getInstance(); 
			// AAA
			case 0x37:	return AAA.getInstance();
			// CMP AL,nn
			case 0x3C:	return new CMP( cpu.AL, nextValue8( proxy ) );
			// CMP AX,nnnn
			case 0x3D:	return new CMP( cpu.AX, nextValue16( proxy ) );
			// DS:
			case 0x3E: 	return DS.getInstance();
			// DAS
			case 0x3F:	return DAS.getInstance();
			// for all others ...
	...
	private OpCode decodeComplexCode( final Intel80x86 cpu, final X86MemoryProxy proxy, final int code8 ) {
		...
		switch( insCode ) {
			case XOR_CODE:	return new XOR( dest, src ); 
			case CMP_CODE:	return new CMP( dest, src );  
