...
public class Decoder90 implements Decoder {
	...
	public OpCode decode( final Intel80x86 cpu, final X86MemoryProxy proxy ) {
		...
		switch( code8 ) {
			// XCHG AX,AX | NOP
			case 0x90: 	return NOP.getInstance(); 
			// XCHG AX,CX
			case 0x91:	return new XCHG( cpu.AX, cpu.CX );
			// XCHG AX,DX
			case 0x92:	return new XCHG( cpu.AX, cpu.DX );
			// XCHG AX,BX
			case 0x93:	return new XCHG( cpu.AX, cpu.BX );
			// XCHG AX,SP
			case 0x94:	return new XCHG( cpu.AX, cpu.SP );
			// XCHG AX,BP
			case 0x95:	return new XCHG( cpu.AX, cpu.BP );
			// XCHG AX,SI
			case 0x96:	return new XCHG( cpu.AX, cpu.SI );
			// XCHG AX,DI
			case 0x97:	return new XCHG( cpu.AX, cpu.DI );
			// CBW/CWDE
			case 0x98: 	return virtualMode ? CWDE.getInstance() : CBW.getInstance();  
			// CWD/CDQ
			case 0x99: 	return virtualMode ? CDQ.getInstance() : CWD.getInstance();  
			// CALL offset, segment
			case 0x9A:	return new CALL( nextAddressFar( cpu, proxy ) );
			// WAIT
			case 0x9B: 	return WAIT.getInstance();  
			// PUSHF
			case 0x9C: 	return virtualMode ? PUSHFD.getInstance() : PUSHF.getInstance();  
			// POPF
			case 0x9D: 	return virtualMode ? POPFD.getInstance()  : POPF.getInstance();  
			// SAHF
			case 0x9E: 	return SAHF.getInstance();  
			// LAHF
			case 0x9F:	return LAHF.getInstance();  
			// unrecognized
