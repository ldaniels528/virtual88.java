...
public class IbmPcRandomAccessMemory {
	...
	/**
	 * Retrieves an operand from memory at the given 
	 * segment and offset of the given size.
	 ...
	 * @return the operand read from memory
	 * @see Operand#SIZE_8BIT
	 * @see Operand#SIZE_16BIT
	 * @see Operand#SIZE_32BIT
	 * @see Operand#SIZE_64BIT
	 */
	public int getOperand( final int segment, final int offset, final int size ) {
		switch( size ) {	
			case SIZE_8BIT:		return getByte( segment, offset );
			case SIZE_16BIT:	return getWord( segment, offset );
			case SIZE_32BIT:	return getDoubleWord( segment, offset );
			case SIZE_64BIT:	return (int)getQuadWord( segment, offset );
			default:
				throw new IllegalStateException( String.format( "Invalid operand size (%d bits)", size ) );
		}
	}
	 ...
	 * @param value the given word
	 */
	public void setQuadWord( final int segment, final int offset, final long value ) {
		  // determine high and low bytes of the word
		  final int hiDword = (int)( ( value & 0xFFFFFFFF00000000L ) >> 32 );
		  final int loDword = (int)( value & 0x00000000FFFFFFFFL );
			
		  // place the double words into memory
		  setDoubleWord( segment, offset, loDword );
		  setDoubleWord( segment, offset + 4, hiDword );
	}
